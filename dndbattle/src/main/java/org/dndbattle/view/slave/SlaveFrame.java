...
public final class SlaveFrame extends AbstractSlaveFrame {
    ...
    private void formComponentMoved(java.awt.event.ComponentEvent evt) {//GEN-FIRST:event_formComponentMoved
        if (getExtendedState() == NORMAL) {
            getSlave().setProperty(SLAVE_LOCATION_X, getLocation().x);
            getSlave().setProperty(SLAVE_LOCATION_Y, getLocation().y);
        }
    }//GEN-LAST:event_formComponentMoved

    private void formComponentResized(java.awt.event.ComponentEvent evt) {//GEN-FIRST:event_formComponentResized
        if (getExtendedState() == NORMAL) {
            getSlave().setProperty(SLAVE_SIZE_WIDTH, getWidth());
            getSlave().setProperty(SLAVE_SIZE_HEIGHT, getHeight());
        }
        if (getExtendedState() != ICONIFIED) {
            getSlave().setProperty(SLAVE_SIZE_STATE, getExtendedState());
        }
    ...
    private void addCombatant(ICombatant combatant, boolean active) {
        ...
        pCombatants.add(new SlaveSubPanel(combatant, checkOwnCharacter(combatant) ? this : null, active));
    }
